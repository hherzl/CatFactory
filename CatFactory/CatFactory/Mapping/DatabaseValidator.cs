        ...
        public virtual ValidationResult Validate(Database database)
        {
            ...
            foreach (var table in database.Tables)
            {
                ...
                foreach (var foreignKey in table.ForeignKeys)
                {
                    ...
                    if (!flag)
                        result.ValidationMessages.Add(new ValidationMessage(LogLevel.Error, string.Format("The table '{0}' has a null reference on foreign key, key: '{1}'", table.FullName, string.Join(",", foreignKey.Key.Select(item => item)))));
                }
            }
